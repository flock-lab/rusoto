...
pub struct CreateFileSystemRequest {
    /// <p><p>Used to create a file system that uses One Zone storage classes. It specifies the Amazon Web Services Availability Zone in which to create the file system. Use the format <code>us-east-1a</code> to specify the Availability Zone. For more information about One Zone storage classes, see <a href="https://docs.aws.amazon.com/efs/latest/ug/storage-classes.html">Using EFS storage classes</a> in the <i>Amazon EFS User Guide</i>.</p> <note> <p>One Zone storage classes are not available in all Availability Zones in Amazon Web Services Regions where Amazon EFS is available.</p> </note></p>
    #[serde(rename = "AvailabilityZoneName")]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub availability_zone_name: Option<String>,
    /// <p><p>Specifies whether automatic backups are enabled on the file system that you are creating. Set the value to <code>true</code> to enable automatic backups. If you are creating a file system that uses One Zone storage classes, automatic backups are enabled by default. For more information, see <a href="https://docs.aws.amazon.com/efs/latest/ug/awsbackup.html#automatic-backups">Automatic backups</a> in the <i>Amazon EFS User Guide</i>.</p> <p>Default is <code>false</code>. However, if you specify an <code>AvailabilityZoneName</code>, the default is <code>true</code>.</p> <note> <p>Backup is not available in all Amazon Web Services Regionswhere Amazon EFS is available.</p> </note></p>
    #[serde(rename = "Backup")]
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct PutAccountPreferencesRequest {
    /// <p><p>Specifies the EFS resource ID preference to set for the user&#39;s Amazon Web Services account, in the current Amazon Web Services Region, either <code>LONG<em>ID</code> (17 characters), or <code>SHORT</em>ID</code> (8 characters).</p> <note> <p>Starting in October, 2021, you will receive an error when setting the account preference to <code>SHORT_ID</code>. Contact Amazon Web Services support if you receive an error and need to use short IDs for file system and mount target resources.</p> </note></p>
...
impl PutAccountPreferencesError {
    pub fn from_response(res: BufferedHttpResponse) -> RusotoError<PutAccountPreferencesError> {
        if let Some(err) = proto::json::Error::parse_rest(&res) {
            match err.typ.as_str() {
                "BadRequest" => {
                    return RusotoError::Service(PutAccountPreferencesError::BadRequest(err.msg))
                }
                "InternalServerError" => {
...
impl fmt::Display for PutAccountPreferencesError {
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            PutAccountPreferencesError::BadRequest(ref cause) => write!(f, "{}", cause),
